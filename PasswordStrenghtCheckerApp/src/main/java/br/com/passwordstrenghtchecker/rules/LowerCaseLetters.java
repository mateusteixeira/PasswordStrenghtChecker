...
public class LowerCaseLetters implements AdditionRule {
	...
	public int analyze(String password) {
		...
		int occurrences = analyzerHelper.getLowerCaseOccurrences(password);

		if (occurrences > 0) {
			score = ((passwordLenght - occurrences) * MULTIPLE_CONSTANT * getAdditionOperator());
		}
